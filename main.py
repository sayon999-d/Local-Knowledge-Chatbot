...
def save_text_to_file(source_name, content):
    try:
        ...
        if "http" in source_name:
            stem = source_name.split("//")[-1]
            secure_name = clean_filename(stem)[:30]
        else:
            secure_name = clean_filename(source_name).rsplit('.', 1)[0][:30]

        filename = f"{secure_name}_{timestamp}.txt"
...
class RAGSystem:
    ...
    def add_url(self, url):
        # SECURITY FIX: Validate URL against SSRF
